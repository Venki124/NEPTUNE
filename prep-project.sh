...
gcloud pubsub topics create neptune-activites

# create the pubsub subscription
gcloud pubsub subscriptions create neptune-activites-test --topic=neptune-activites
...
gcloud functions deploy function_pb_bq --gen2 --region=us-central1 --runtime=python312 --trigger-topic=neptune-activites --entry-point=pubsub_to_bigquery --memory=256MB
